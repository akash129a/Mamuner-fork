...
module.exports = {
	...
	onReply: async function ({ api, event, Reply, message, threadsData }) {
		...
		if (state === "main") {
			if (num === 1) {
				const menu = [
					...
					`2. Auto Restart (set time) — current: ${config.autoRestart?.time ?? "null"}`,
			...
			if (num === 8) {
				...
				const current = threadData.data?.onlyAdminBox === true;
				threadData.data = threadData.data || {};
				threadData.data.onlyAdminBox = !current;
				await threadsData.set(event.threadID, threadData.data, "data");
				return message.reply(`✦ Only Admin Box (এই গ্রুপে) — ${status(!current)}`);
			}
		}
		...
		if (state === "botConfig") {
			...
			if (num === 2) {
				return await sendAndListen("› Reply with restart time in ms (e.g. 3600000 for 1hr), or 'null' to disable:", "autoRestartSet");
			}
			if (num === 3) {
		...
		if (state === "autoRestartSet") {
			config.autoRestart = config.autoRestart || {};
			config.autoRestart.time = input === "null" ? null : (isNaN(input) ? null : parseInt(input));
			saveConfig();
